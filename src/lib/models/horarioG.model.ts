...
export class HorarioG {
  ...
  renderizarGrafico(parametrosHorario: ConfiguracionSemana, plantilla?: Plantilla) {
  ...
  //----------------------------------------------------------------------------------------------------------
  generarGrafico(parametrosHorario: ConfiguracionSemana, plantillaActual?: Plantilla) {
  ...
  private configurarSvg()
  {
    ...
    this.svg
      .attr('width', this.configuracion.grafico.anchoGrafico)
      .attr('height', this.configuracion.grafico.altoGrafico)
    ...
    this.svg.append('rect')
      ...
      .attr('fill', this.configuracion.grafico.colorGrafico)
      .attr('rx', '10')
      .attr('ry', '10')
  ...
  private inicializarParametros(parametrosHorario: ConfiguracionSemana) {

    this.configuracion.configuracionSemana = parametrosHorario;

    // Cálculo del rango en horas del horario
    const fechaFin: Date = HorarioG.convertirCadenaHoraEnTiempo(this.configuracion.configuracionSemana.horaMaxima);
    const fechaInicio: Date = HorarioG.convertirCadenaHoraEnTiempo(this.configuracion.configuracionSemana.horaMinima);
    ...
    this.configuracion.grafico.anchoGrafico = parseFloat(d3.select(this.elementoRaiz).style('width'));
    this.configuracion.grafico.altoGrafico = parseFloat(d3.select(this.elementoRaiz).style('height')) * Math.max(1, rangoEnHoras / 7);


    // Establecer dimensiones del panel que contiene las barras.
    this.configuracion.panelHorario.anchoPanelHorario  = this.configuracion.grafico.anchoGrafico * ((100-this.configuracion.grafico.margenGrafico.margenIzquierdoGrafico - this.configuracion.grafico.margenGrafico.margenDerechoGrafico)/100);
    this.configuracion.panelHorario.altoPanelHorario   = this.configuracion.grafico.altoGrafico * ((100-this.configuracion.grafico.margenGrafico.margenSuperiorGrafico - this.configuracion.grafico.margenGrafico.margenInferiorGrafico)/100);

    // Establecer escala horizontal: Serán bandas que identifiquen a los días de la semana
    this.configuracion.escalas.escalaHorizontal = d3.scaleBand()
      .domain(this.obtenerDiasSemanaHorario().map(ds=> ds.denominacionLarga))
      .range([0, this.configuracion.panelHorario.anchoPanelHorario])
      .paddingInner(0.0)
      .paddingOuter(0.0);

    // Establecer escala vertical:
    this.configuracion.escalas.escalaVertical = d3.scaleTime()
      .domain([this.minimoIntervaloTemporal(), this.maximoIntervaloTemporal()])
      .range([0, this.configuracion.panelHorario.altoPanelHorario])

    // Calcular el ancho de las sesiones.
    this.configuracion.panelSesiones.anchoSesion = parseFloat(this.configuracion.escalas.escalaHorizontal.bandwidth()) * (100-this.configuracion.panelSesiones.margenLateral * 2)*0.01;
  ...
  private anyadirPanelHorario() {
    ...
    //-------------------------------------------------
    const coordenadaXPanel = this.configuracion.grafico.anchoGrafico as number * (this.configuracion.grafico.margenGrafico.margenIzquierdoGrafico / 100);
    const coordenadaYPanel = this.configuracion.grafico.altoGrafico as number * (this.configuracion.grafico.margenGrafico.margenSuperiorGrafico / 100);
    ...
    panelHorario.append('rect')
      ...
      .attr('width', this.configuracion.panelHorario.anchoPanelHorario)
      .attr('height', this.configuracion.panelHorario.altoPanelHorario)
      .attr('fill', this.configuracion.panelHorario.colorPanelHorario);

    //-------------------------------------------------
    // Adicón del eje X
    //-------------------------------------------------
    var ejeX = d3.axisTop(this.configuracion.escalas.escalaHorizontal as d3.ScaleBand<string>);
    ...
    // Adición del eje Y
    //-------------------------------------------------
    var ejeY = d3.axisLeft(this.configuracion.escalas.escalaVertical);
  ...
  private anyadirPanelesDiasSemana() {
    ...
    panelesDiasSemana.merge(aux)
      ...
      .attr('transform', (d: DiaSemana) => `translate(${this.configuracion.escalas.escalaHorizontal ? this.configuracion.escalas.escalaHorizontal(d.denominacionLarga) : 0},0)`);

    panelesDiasSemana.exit().remove();

    panelesDiasSemana
      .append('line')
      .attr('x1', this.configuracion.escalas.escalaHorizontal?.bandwidth)
      .attr('y1', 0)
      .attr('x2', this.configuracion.escalas.escalaHorizontal?.bandwidth)
      .attr('y2', this.configuracion.panelHorario.altoPanelHorario as number)
  ...
  renderizarSesiones(panelDiaSemana: string, sesiones: Sesion[]) {
    ...
    const anchoSesion = this.configuracion.panelSesiones.anchoSesion ? this.configuracion.panelSesiones.anchoSesion.toString() : '0';

    // Definición del panel
    const panelSesion = d3.select(panelDiaSemana).selectAll('g#sesion' + 'pp').data(sesiones).enter().append('g')
      .attr('transform', d => `translate(${ this.configuracion.panelSesiones.margenLateral  },${this.configuracion.escalas.escalaVertical(HorarioG.convertirCadenaHoraEnTiempo(d.horaInicio))})`)
    ...
    panelCabeceraSesion.append('rect')
      ...
      .attr('height',this.configuracion.panelSesiones.altoCabecera)
      .attr('width', anchoSesion)
      .attr('fill', this.configuracion.panelSesiones.colorCabecera);

    panelCabeceraSesion.append('text')
      .attr('x', parseInt(anchoSesion) / 2)
      .text(d => d.horaInicio + ' - ' + d.horaFin)
      .attr('y', this.configuracion.panelSesiones.altoCabecera / 2)
      .attr('font-size','.5em')
    ...
    const panelCuerpoSesion = panelSesion.append('g')
      .attr('class', 'panelCabeceraSesion')
      .attr('transform', d => `translate(0,${this.configuracion.panelSesiones.altoCabecera})`);
    ...
    panelCuerpoSesion.append('rect')
      ...
      .attr('height', (d: Sesion) => {
        const coordenadaHoraInicio = this.configuracion.escalas.escalaVertical(HorarioG.convertirCadenaHoraEnTiempo(d.horaInicio));
        const coordenadaHoraFin = this.configuracion.escalas.escalaVertical(HorarioG.convertirCadenaHoraEnTiempo(d.horaFin));
        return coordenadaHoraFin - coordenadaHoraInicio - this.configuracion.panelSesiones.altoCabecera;
      })
      .attr('width', anchoSesion)
      .attr('fill', this.configuracion.panelSesiones.colorCuerpo)
      // .attr('stroke', 'black')
      // .attr('stroke-width', '1');
  ...
  crearPanelesSesionActividades(panelDiaSemana: string, actividadesSesion: IActividadesSesion[]) {
    const panelSesionActividades = d3.select(panelDiaSemana)
      ...
      .enter()
      .append('g')
      .attr('transform', d => `translate(0,${this.configuracion.escalas.escalaVertical(HorarioG.convertirCadenaHoraEnTiempo(d.sesion.horaInicio))})`)
    ...
    //   .attr('height', d => {
    //     const coordenadaHoraInicio = this.configuracion.escalas.escalaVertical(HorarioG.convertirCadenaHoraEnTiempo(d.sesion.horaInicio));
    //     const coordenadaHoraFin = this.configuracion.escalas.escalaVertical(HorarioG.convertirCadenaHoraEnTiempo(d.sesion.horaFin));
    //     return coordenadaHoraFin - coordenadaHoraInicio;
    //   })
    //   .attr('width', d => this.configuracion.escalas.escalaHorizontal.bandwidth())
    //   .attr('fill', 'red')
    //   .attr('opacity', '0.4')
    //   .attr('rx', 2)
    //   .attr('ry', 2);
  ...
  crearPanelCabeceraSesionConActividades(panelSesionActividades: any) {
    const anchoSesion = this.configuracion.panelSesiones.anchoSesion ? this.configuracion.panelSesiones.anchoSesion.toString() : '0';
    const altoCabeceraSesion = this.configuracion.panelSesiones.altoCabecera;
  ...
  crearPanelCuerpoSesionConActividades(panelSesionActividades: any) {
    ...
    panelCuerpoSesionConActividades.append('rect')
      ...
      .attr('width', (d: any) => this.configuracion.escalas.escalaHorizontal.bandwidth())
      .attr('fill', 'yellow')
      .attr('opacity', '0')
      .attr('rx', 2)
      .attr('ry', 2);
  ...
  añyadirPanelesActividades(actividadesSesiones: IActividadesSesion[]) {
    ...
    actividadesSesiones.forEach(as => {
      ...
      const altoPanelActividadesEnActividadSesiones = this.altoPanel(as.sesion)-this.configuracion.panelSesiones.altoCabecera;
  ...
  public obtenerDiasSemanaHorario(): DiaSemana[] {
    ...
    return HorarioG.diasSemana.filter((ds: DiaSemana) => this.configuracion.configuracionSemana?.diasSemanaHabiles.includes(ds.codigo) );
  ...
  public minimoIntervaloTemporal(): Date {
    const horaMinima = HorarioG.convertirCadenaHoraEnTiempo(this.configuracion.configuracionSemana?.horaMinima);
    return horaMinima.setMinutes(horaMinima.getMinutes());
  }

  public maximoIntervaloTemporal(): Date {
    const horaMaxima = HorarioG.convertirCadenaHoraEnTiempo(this.configuracion.configuracionSemana?.horaMaxima);
  ...
  private altoPanel(sesion: Sesion) {
    const coordenadaHoraInicio = this.configuracion.escalas.escalaVertical(HorarioG.convertirCadenaHoraEnTiempo(sesion.horaInicio));
    const coordenadaHoraFin = this.configuracion.escalas.escalaVertical(HorarioG.convertirCadenaHoraEnTiempo(sesion.horaFin));
